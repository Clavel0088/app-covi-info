...
<meta name="Description" content="<?php if (! empty($actus) && is_array($actus)) {
	echo $vaccinActus[0]['titre'];
} ?>" />
<meta http-equiv="X-UA-Compatible" content="IE=edge"/>
<title><?php if (! empty($vaccinActus) && is_array($vaccinActus)) { echo $vaccinActus[0]['titre']; } ?></title>
...
<div class="site-wrapper">
	...
	<div class="vertical">
		<div class="main-header-content inner">
			<h1 class="post-title"><?= $vaccinActus[0]['titre'] ?></h1>
			<div class="entry-title-divider">
				<span></span><span></span><span></span>
			</div>
			<section class="post-meta">
			<time class="post-date" datetime="2015-12-13"><?php if (! empty($vaccinActus) && is_array($vaccinActus)) { echo $vaccinActus[0]['date']; } ?> </time> | <a class="scrolltocomments" href="#">Vaccins</a>
			</section>
		</div>
	</div>
	</header>
	...
	<div class="wraps">
		...
		<?php if (! empty($vaccinActus) && is_array($vaccinActus)) : ?>
		<h2><?= $vaccinActus[0]['titre'] ?></h2> 
		<p>
			<?php if (!empty($keywords) && is_array($keywords)) : 
				foreach ($keywords as $row) :
					$motcle=$row['mot'];
					$motremplace="<strong>".$motcle."</strong>";
					$vaccinActus[0]['description']=str_ireplace($motcle,$motremplace , $vaccinActus[0]['description']);
				endforeach;
			?>
			<?php endif ?>
			<?php echo $vaccinActus[0]['description']; ?>
		</p>
		<?php endif ?>
		</section>
